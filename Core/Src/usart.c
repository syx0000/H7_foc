...
{
    if (printf_len < PRINTF_BUF_SIZE)
    {
        printf_buf[printf_len++] = ch;
    }
...
// ISR-safe print function (non-blocking, drops data if busy)
void isr_print(const char *str)
{
    static uint8_t isr_buf[128];
    uint16_t len = 0;

    while (*str && len < sizeof(isr_buf)) {
        isr_buf[len++] = *str++;
    }
